class RequirementChecker:
    ...
    def check_pip(self):
        ...
        import click
        try:
            # Try running python3 and python commands for pip check
            checkpy3 = subprocess.run(['python3', '-m', 'pip', '--version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except FileNotFoundError:
            checkpy3 = None

        try:
            check = subprocess.run(['python', '-m', 'pip', '--version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except FileNotFoundError:
            check = None

        # Check if pip3 or pip are missing based on the return codes
        if not shutil.which("pip3") and not shutil.which("pip") and (checkpy3 is None or checkpy3.returncode != 0) and (check is None or check.returncode != 0):
    ...
    def check_virtualenv(self):
        ...
        try:
            # Try running python3 and python commands for virtualenv check
            checkpy3 = subprocess.run(['python3', '-m', 'virtualenv', '--version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except FileNotFoundError:
            checkpy3 = None

        try:
            check = subprocess.run(['python', '-m', 'virtualenv', '--version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except FileNotFoundError:
            check = None

        # Check if virtualenv is missing
        if not shutil.which("virtualenv") and (checkpy3 is None or checkpy3.returncode != 0) and (check is None or check.returncode != 0):
